...
log_results, logs_path = True, "HS-clear/HS_CLEAR_SST_fc_{}_inp_{}d_knn_linf/HS_CLEAR_results_smallest_expl_SST_{}fc_{}_inp_{}d".format(num_words, emb_dims, prefix, num_words, emb_dims)  # write results on file
...
"""
# this has to be removed (fast proxy to try epsilon version)
for i in range(len(minmax_input_bounds)):
    for j in range(len(minmax_input_bounds[i])):
        for n in range(len(minmax_input_bounds[i][j])):
            if j == 0:
                minmax_input_bounds[i][j][n] = x[0,(i*window_size)+n]-eps
            else:
                minmax_input_bounds[i][j][n] = x[0,(i*window_size)+n]+eps 
#
"""
