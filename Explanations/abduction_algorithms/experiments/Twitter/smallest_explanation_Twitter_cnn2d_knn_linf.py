...
parser.add_argument('-k', '--kneighbors', dest='knn', type=int, default=8, help='Number of k-nearest-neighbors used to find the convex hull around each word from the input sequence')
parser.add_argument('-a', '--adv', dest='adv_attacks', type=int, default=30, help='Number of adversarial attacks (0 means disabling )')
...
ksize, emb_dims = int(num_words**0.5), window_size
...
log_results, logs_path = True, "HS-clear/HS_CLEAR_SST_cnn_{}_inp_{}d_knn_linf/HS_CLEAR_results_smallest_expl_SST_{}cnn_{}_inp_{}d".format(num_words, emb_dims, prefix, num_words, emb_dims)  # write results on file
...
if log_results is True:
    ...
    logs = "Input: {}\nKNN, Window Size, Adv_attacks: {}, {}, {}\nOriginal output (confidence): {} ({})\nMinimal Expl: {} ({} words)\nExec Time: {}\n\n".format(input_without_padding, knn, window_size, (True if adv_sims>0 else False), y_hat, c_hat, h, int(len(h)/window_size), exec_time)
    file_.write(logs)
    file_.close()
